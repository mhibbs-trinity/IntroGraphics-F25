...
Sphere centralSphere = new Sphere(new PVector(0,0,0), 150, 24);
Sphere leftSphere = new Sphere(new PVector(-120,0,0), 100, 15);
...
void mouseDragged() {
  popMatrix();
  float rotY = (mouseX - pmouseX) * 0.01;
  float rotX = (mouseY - pmouseY) * 0.01;
  rotateX(rotX);
  rotateY(rotY);
  pushMatrix();
}
